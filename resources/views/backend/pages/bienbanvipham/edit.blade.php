        ...
        <div class="white-box">
            <form id="bienbanForm" action="{{route('phieumuon.store')}}" method="post">
                ...
                <div style="margin-top:2rem ;margin-right:1rem ;">
                    ...
                    value="{{$vipham->phieumuon()->first()->id}}">
                </div>
                ...
                <div style="margin-top:2rem ;margin-right:1rem ;">
                    ...
                    <input type="text" disabled class="form-control" value="{{$vipham->phieumuon()->first()->docgia()->first()->hoTen}}"  name="maDocGia" id="maDocGia">
                </div>

                <div style="margin-top:2rem ;margin-right:1rem ;">
                    <label style="display: block" for="tienPhat">Tiền phạt</label>
                    <input style="display: inline-block;width:30%" value="{{$vipham->tienPhat}}" type="text" class="form-control" required name="tienPhat" id="tienPhat">
                    <label> VND</label>
                </div>

                <div style="margin-top:2rem ;margin-right:1rem ;">
                    <label for="noiDung">Nội dung ghi chú</label>
                    <textarea class="form-control" name="noiDung" id="noiDung" style="height: 150px">{{$vipham->noiDung}}</textarea>
                </div>
...
<script>
    ...
    $("#check").click(function () {
        ...
        var id = href.toString().split('vipham/')[1].split('/')[0];
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response['yes'] == true) {
                    alertify.success('Sửa biên bản thành công');
                }
            }
        });
    }
