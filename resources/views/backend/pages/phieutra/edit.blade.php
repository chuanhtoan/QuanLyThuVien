        ...
        <div class="white-box">
            <form id="phieutraForm" action="{{route('phieutra.store')}}" method="post">
                ...
                <div style="margin-top:2rem ;margin-right:1rem ;">
                    ...
                    <input type="text" class="form-control" disabled name="maPhieuMuon" id="maPhieuMuon"
                    value="{{$item->ID_PhieuMuon}}">
                </div>
                ...
                <div>
                    ...
                    value="{{$item->ngayTra->format('Y-m-d')}}">
                </div>
                ...
                <div style="margin-top:2rem ;margin-right:1rem ;">
                    ...
                    <select class="form-control" id="selectNhanVien" name="selectNhanVien" required>
                        ...
                        <option 
                        @if ($items->id == $item->ID_NhanVien)
                            selected
                        @endif  
                        value="{{$items->id}}">{{$items->hoTen}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="chitietphieumuon" style="margin-top: 5rem; ">
                    ...
                    <div class="table-responsive" id="tb_tl">
                        ...
                        <table class="table" id='table-chitiet'>
                            ...
                            <tbody>
                                @foreach ($item->cuonsachs()->get() as $i)
                                     @include('backend.pages.sach.row_chitiet',['item'=>$i])
                                @endforeach
                            </tbody>
                        </table>
                    </div>
...
<script>
   ...
   // ===============  PHAN VALIDATE DU LIEU   ================ //
    ...
    $(document).ready(function (e) {
        $('#khong-ton-tai').hide();
        var mang = $('#table-chitiet tbody tr td:first-child()');
    ...
    function hienViPham(i, data,kt) {
        ...
        if (i == data['size']) {
            alertify.confirm(
                'Bạn có muốn tiếp tục sửa phiếu mượn?',
                ()=> { 
                    var href =   window.location;
                    var id =href.toString().split('phieutra/')[1].split('/')[0] ;
                    thucHienAjax(id);
